...
async def register_todo(data:dict) -> Union[dict, bool]:
...
async def get_single_todo(id:str) -> Union[dict, bool]:
    todo = await collection_todo.find_one({'_id': ObjectId(id)})
    if todo:
        return todo_serializer(todo)
    return False

async def update_todo(id:str, data:dict) -> Union[dict, bool]:
    todo = await collection_todo.find_one({'_id': ObjectId(id)})
    if todo:
